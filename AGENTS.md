...
> **`CLAUDE.md` y `AGENTS.md` son el mismo documento**, copiado en dos nombres
> porque cada herramienta lee el suyo. Si tocas uno, copia el otro — sólo
> cambia la primera línea. Comprobación: `diff CLAUDE.md AGENTS.md` debe dar
> exactamente esa línea y ninguna más.
...
barbero desde el panel, uno a uno.

**El correo sí es 100 % automático** (Resend), y el de confirmación lleva un
`.ics` adjunto con alarma 2 h antes.

### Qué botón sale y cuándo — lo decide el ESTADO, no la pestaña

`src/components/AvisosWhatsApp.tsx` es el único sitio que elige plantilla, y la
elige mirando `cita.estado`:

| Estado | Botones |
| --- | --- |
| `en_revision` | «Avisar: comprobante en revisión» — recibido, **todavía no aprobado** |
| `confirmada` | «Enviar confirmación» + «Recordar fecha y hora» |
| `pendiente_pago` | «Avisar: corregir comprobante» (el rechazo) |
| `atendida`, `no_show`, `cancelada`, `liberada` | ninguno: no se confirma una cita cerrada |

Hasta 2026-09-10 los enlaces colgaban de la pestaña: recordatorio sólo en
«Mañana», confirmación sólo en «Próximas». Eso dejaba dos huecos reales — el
cliente que acababa de subir su captura no recibía nada, y una cita aprobada
para dentro de tres días no tenía botón de recordar hasta la víspera. Por eso
va por estado y no por pestaña: **la misma tarjeta ofrece lo mismo la mires
donde la mires.**

Y tras aprobar o rechazar, el panel deja un aviso arriba con el botón ya
apuntando a ese cliente. Sin él la cita salta a otra pestaña justo cuando lo
siguiente que quieres hacer es avisarle.

El panel **nunca lista citas en `pendiente_pago`** (mira la consulta de
`panel/page.tsx`): esa rama sólo se alcanza desde el aviso posterior a un
rechazo. Si algún día se listan, revisa que a una cita recién creada —que aún
no ha pagado— no se le ofrezca un mensaje que dice «no pude validar tu
captura».

Abrir un chat de `wa.me` **no demuestra que se envió nada**, así que ninguna
cita se marca como «avisada». El registro de lo que sí salió está en
`notificaciones`, y ahí sólo entra el correo.
...
### Mobile-first, y luego crece

La mayoría de las reservas entran desde un celular con 4G, así que **el diseño
base —sin ningún prefijo de breakpoint— es el del móvil**, y es el que manda:
...
Mobile-first **no** quiere decir «sólo móvil». Hasta 2026-09-06 no había ni un
breakpoint en todo el proyecto: en un portátil la web era una columna de 440 px
en mitad de una pantalla de 1900. Ahora sí los hay, y todos son `sm:` / `md:` /
`lg:`, es decir, **aditivos hacia arriba**: por debajo de 640 px no cambia
absolutamente nada. Si tocas esto, mantén esa propiedad.

Dónde crece y por qué:

| Sitio | Móvil | Ensanchado |
| --- | --- | --- |
| Contenedor del flujo | 440 px | `sm` 680 px |
| Servicios | una columna | `sm` dos columnas |
| Barberos | dos columnas | tope `sm:max-w-[420px]` — son dos, sin tope salen dos carteles |
| Franja de días | scroll horizontal | `sm` se envuelve en varias filas: arrastrar una barra con ratón es incómodo |
| Rejilla de horas | 4 columnas | `sm` 6, `md` 8 — de 11 filas a 6 |
| Datos y pago | ancho completo | tope 480/520 px: un input de 680 px se lee peor |
| Pantalla de pago | en columna | `md` dos columnas — el número de Yape y el botón de subir, a la vez |
| Panel del barbero | una columna, 560 px | `lg` dos columnas, 1120 px |

**Lo que NO crece, a propósito:** el **ticket** se queda en 360 px en cualquier
pantalla. Es un recibo y está dimensionado para que se lea en la miniatura de un
chat de WhatsApp; además `--corte` está cuadrado a mano con la altura de su
cabecera, así que si cambias su ancho o sus tamaños de letra, revisa que las
muescas sigan alineadas con la línea perforada.

Ojo al verificar: `resize_window` de las herramientas de navegador **no hace
nada si la ventana de Chrome está maximizada** (sigue informando de éxito), y la
CSP del proyecto impide meter la web en un `<iframe>` para medirla. Para
comprobar un ancho concreto hace falta desmaximizar la ventana a mano.
...
### g) Brevo trae la restricción por IP activada, y con Vercel es incompatible

Al poner la primera `BREVO_API_KEY` (2026-09-06), **cualquier** llamada a la API
—incluso un `GET /v3/account` de sólo lectura— respondía 401:

```
We have detected you are using an unrecognised IP address 190.232.219.158.
```

No es la clave: es un ajuste de la cuenta, en
`app.brevo.com/security/authorised_ips`. Mientras esté activo no sale ni un
correo, y el síntoma es idéntico al de las comillas de la sección (f) — se anota
`fallido` en `notificaciones` y la reserva sigue como si nada.

**Añadir la IP no arregla nada.** Quien llama a Brevo en producción no es tu
portátil, es una función de Vercel, y sus IP de salida son dinámicas: no hay
lista que mantener. La restricción hay que **desactivarla**, no ampliarla.

**Regla:** cuando conectes un proveedor nuevo, prueba primero su endpoint más
tonto de sólo lectura (`GET /account`, `/me`, `/ping`). Si la cuenta tiene un
cepo de este tipo, sale ahí en dos segundos en vez de aparecer como «no llegan
los correos» tres días después.
...
src/components/
  ...
  PanelBarbero.tsx         4 pestañas
  AvisosWhatsApp.tsx       Qué enlace wa.me toca según el estado de la cita
  ui.tsx                   Primitivas

pruebas/
  librerias.ts             `test:lib`       — puro, sin red
  interfaz.tsx             `test:interfaz`  — renderiza los componentes en Node
  galeria-whatsapp.mjs     `test:navegador` — Edge real, 360 y 1280 px
  e2e.mjs                  `test:e2e`       — contra `dev:pruebas`
  panel.mjs                `test:panel`     — login real + RLS
  anti-solape.sql          A mano, en el SQL Editor de Supabase

scripts/                   Los 3 crons
.github/workflows/         Sus disparadores
```

### La subida de la captura NO fuerza la cámara

El `<input type="file">` del paso 5 lleva `accept` pero **no `capture`**. Con
`capture="environment"` —como estaba hasta 2026-09-10— Android y iOS abren la
cámara directamente y **no dejan llegar al carrete**. Es exactamente lo
contrario de lo que hace falta: la captura del Yape ya está guardada en la
galería, nadie la va a fotografiar. Si vuelves a añadir `capture`, rompes el
flujo entero; hay una prueba en `test:interfaz` que lo fija.
...
npm run test:interfaz # los componentes renderizados: galería, avisos, plantillas
npm run test:navegador # si tocaste FlujoReserva o PanelBarbero
...
`test:navegador` levanta los componentes reales con esbuild + el CSS real de
Tailwind y los conduce con **playwright-core sobre el Edge que ya trae
Windows** (`channel: 'msedge'`). Es `playwright-core` y no `playwright` a
propósito: el paquete grande descarga ~500 MB de navegadores en el postinstall
y aquí no hace falta ni uno. No toca la red ni Supabase — intercepta
`/api/captura` y `/api/panel/validar`— así que se puede correr siempre.

Es la única prueba que ve lo que ve el cliente: que el selector de archivo
abre la galería, que el número de Yape y el titular se leen, y que no hay
scroll horizontal a 360 px. `typecheck` y `build` no detectan nada de eso.
...
**Pero `dev:pruebas` NO sirve para probar a mano con un navegador.** El secreto
de prueba («siempre pasa») da por buenos los tokens *falsos* de los scripts, y
rechaza los *reales* que emite el widget: el widget usa
`NEXT_PUBLIC_TURNSTILE_SITE_KEY`, que es la clave de verdad, y Cloudflare empareja
sitio y secreto. Se ve como «No pudimos verificar que eres una persona» en el
paso 4, con el widget en verde justo encima. No es un fallo del código.

  · script / `npm run test:e2e` → `npm run dev:pruebas`
  · tú con un navegador          → `npm run dev`
